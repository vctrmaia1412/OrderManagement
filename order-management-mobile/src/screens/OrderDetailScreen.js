...
function showAlert(title, msg) { if (Platform.OS === 'web') window.alert(`${title}\n${msg}`); }
function showConfirm(msg) { if (Platform.OS === 'web') return window.confirm(msg); return true; }
...
export default function OrderDetailScreen({ route, navigation }) {
  ...
  const fetchOrder = async () => { try { const { data } = await orderService.getById(orderId); setOrder(data); } catch {} finally { setLoading(false); } };
  ...
  const handleApprove = async () => {
    if (!showConfirm(`${t('confirmApprove')} #${orderId}?`)) return;
    try { setActionLoading(true); await orderService.approve(orderId); showAlert(t('success'), t('orderApproved')); fetchOrder(); }
    catch (e) { showAlert(t('error'), e.response?.data?.message || t('approveError2')); }
    finally { setActionLoading(false); }
  };

  const handleCancel = async () => {
    if (!showConfirm(`${t('confirmCancel')} #${orderId}?`)) return;
    try { setActionLoading(true); await orderService.cancel(orderId); showAlert(t('success'), t('orderCanceled')); fetchOrder(); }
    catch (e) { showAlert(t('error'), e.response?.data?.message || t('cancelError')); }
    finally { setActionLoading(false); }
  };
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={{ padding: 16 }}>
      ...
      <View style={styles.section}>
        ...
        <Row label={t('date')} value={fmtDate(order.orderDate)} />
        <Row label={t('total')} value={fmt(order.totalAmount)} bold />
      ...
      {order.deliveryTerm && (
        <View style={styles.section}>
          ...
          <Row label={t('deliveryEstimate')} value={fmtDate(order.deliveryTerm.estimatedDeliveryDate)} />
        </View>
      )}
      <View style={styles.section}>
...
function Row({ label, value, bold }) {
  return (<View style={styles.row}><Text style={styles.rowLabel}>{label}</Text><Text style={[styles.rowValue, bold && { fontWeight: 'bold', fontSize: 18, color: '#4338ca' }]}>{value}</Text></View>);
}

const styles = StyleSheet.create({
  container: { flex: 1, backgroundColor: '#f3f4f6' }, center: { flex: 1, justifyContent: 'center', alignItems: 'center' },
  headerRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 },
  title: { fontSize: 22, fontWeight: 'bold', color: '#1f2937' }, badge: { paddingHorizontal: 12, paddingVertical: 5, borderRadius: 20 },
  badgeText: { fontSize: 12, fontWeight: '600' }, section: { backgroundColor: '#fff', borderRadius: 12, padding: 16, marginBottom: 12, elevation: 1 },
  sectionTitle: { fontSize: 11, fontWeight: '700', color: '#6b7280', letterSpacing: 1, marginBottom: 10 },
  row: { flexDirection: 'row', justifyContent: 'space-between', paddingVertical: 6 }, rowLabel: { fontSize: 14, color: '#6b7280' },
  ...
  itemName: { fontSize: 14, fontWeight: '500', color: '#1f2937' }, itemSub: { fontSize: 12, color: '#6b7280', marginTop: 2 },
  itemTotal: { fontSize: 14, fontWeight: '600', color: '#1f2937' }, actions: { marginTop: 8, marginBottom: 30 },
  actionBtn: { borderRadius: 10, paddingVertical: 14, alignItems: 'center', marginBottom: 10, cursor: 'pointer' },
  disabled: { opacity: 0.6 }, actionText: { color: '#fff', fontWeight: '600', fontSize: 15 },
});
