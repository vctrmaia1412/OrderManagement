...
export default function ApprovalQueueScreen({ navigation }) {
  ...
  const fetchPending = async () => { try { const { data } = await orderService.getPending(); setOrders(data); } catch {} finally { setLoading(false); } };
  useEffect(() => { fetchPending(); }, []);

  const handleApprove = async (e, orderId) => {
    if (e?.stopPropagation) e.stopPropagation();
    if (Platform.OS === 'web' && !window.confirm(`${t('confirmApprove')} #${orderId}?`)) return;
    try { setApproving(orderId); await orderService.approve(orderId); await fetchPending(); }
    catch { if (Platform.OS === 'web') window.alert(t('approveError')); }
  ...
  return (
    <View style={{ flex: 1 }}>
      <View style={styles.toolbar}>
        ...
        <TouchableOpacity style={styles.refreshBtn} onPress={fetchPending}><Text style={styles.refreshText}>{t('refresh')}</Text></TouchableOpacity>
      </View>
      <FlatList data={orders} keyExtractor={(item) => String(item.orderId)} renderItem={renderItem} contentContainerStyle={styles.list}
        ListEmptyComponent={<View style={styles.center}><Text style={styles.emptyIcon}>✅</Text><Text style={styles.empty}>{t('noPending')}</Text></View>}
      />
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  list: { padding: 16 }, center: { flex: 1, justifyContent: 'center', alignItems: 'center', padding: 40 },
  ...
  orderId: { fontSize: 16, fontWeight: 'bold', color: '#1f2937' }, createdBy: { fontSize: 12, color: '#6b7280', marginLeft: 8 },
  customer: { fontSize: 14, color: '#374151', marginBottom: 4 }, total: { fontSize: 18, fontWeight: 'bold', color: '#4338ca' },
  approveBtn: { backgroundColor: '#059669', paddingHorizontal: 20, paddingVertical: 14, borderRadius: 10, marginLeft: 12, cursor: 'pointer' },
  approveBtnDisabled: { backgroundColor: '#9ca3af' }, approveBtnText: { color: '#fff', fontSize: 14, fontWeight: '700' },
  empty: { fontSize: 15, color: '#9ca3af', marginTop: 8 }, emptyIcon: { fontSize: 40 },
});
